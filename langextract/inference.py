...
class ScoredOutput:
  """Represents a language model's output with an associated score.

  This dataclass is used to encapsulate the text output generated by a language
  model along with a confidence score, allowing for ranking or selection of
  the most probable outputs.
  """
...
class InferenceOutputError(exceptions.LangExtractError):
  """Exception raised when language model inference fails to produce valid output.

  This error indicates that the language model was unable to generate any
  scored outputs, possibly due to issues with the prompt, model configuration,
...
class BaseLanguageModel(abc.ABC):
  """An abstract base class for managing language model inference.

  This class defines the interface for all language models in this library.
  It provides a common structure for performing inference, regardless of the
  underlying model provider (e.g., Ollama, Gemini, OpenAI).

  Subclasses must implement the `infer` method to provide the specific
  inference logic for their respective models.
...
class OllamaLanguageModel(BaseLanguageModel):
  ...
  def infer(
    ...
    """Runs inference on a list of prompts using the Ollama model.

    This method iterates through the provided prompts and sends each one to the
    Ollama model for inference. It yields a single `ScoredOutput` for each
    prompt, with a default score of 1.0.
...
class GeminiLanguageModel(BaseLanguageModel):
  ...
  def infer(
    ...
    This method uses a thread pool to process multiple prompts in parallel,
    making it efficient for batch inference. It yields a single `ScoredOutput`
    for each prompt.
  ...
  def parse_output(self, output: str) -> Any:
    ...
    This method is responsible for parsing the raw output from the Gemini API
    into a Python object. It supports both JSON and YAML formats.
...
class OpenAILanguageModel(BaseLanguageModel):
  ...
  def parse_output(self, output: str) -> Any:
    ...
    This method is responsible for parsing the raw output from the OpenAI API
    into a Python object. It supports both JSON and YAML formats.
